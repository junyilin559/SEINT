...
def RIOT(
    ...
    if(acc):
        ...
        X2 = -2*Xn @ (Xn.T @ X1) + np.outer(X_plength2 @ X1 , np.ones(len(X_plength2))).T + np.outer(np.ones(len(X_plength2)), X_plength2 @ X1)
        Y2 = -2*Yn @ (Yn.T @ Y1) + np.outer(Y_plength2 @ X1 , np.ones(len(Y_plength2))).T + np.outer(np.ones(len(Y_plength2)), Y_plength2 @ X1)
    else:
