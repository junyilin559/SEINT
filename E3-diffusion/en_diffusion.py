...
## adds
from equivariant_diffusion import RIOT_batch
...
def gaussian_entropy(mu, sigma):
    ...
    return sum_except_batch(zeros + 0.5 * torch.log(2 * np.pi * sigma**2) +
                            0.5)
...
def gaussian_KL(q_mu, q_sigma, p_mu, p_sigma, node_mask):
    ...
    return sum_except_batch(
        (torch.log(p_sigma / q_sigma) + 0.5 * (q_sigma**2 + (q_mu - p_mu)**2) /
         (p_sigma**2) - 0.5) * node_mask)
...
def gaussian_KL_for_dimension(q_mu, q_sigma, p_mu, p_sigma, d):
    ...
    return d * torch.log(p_sigma / q_sigma) + 0.5 * (
        d * q_sigma**2 + mu_norm2) / (p_sigma**2) - 0.5 * d
...
class GammaNetwork(torch.nn.Module):
    ...
    def forward(self, t):
        ...
        # Normalize to [0, 1]
        normalized_gamma = (gamma_tilde_t - gamma_tilde_0) / (gamma_tilde_1 -
                                                              gamma_tilde_0)
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def normalize(self, x, h, node_mask):
        ...
        h_cat = (h['categorical'].float() -
                 self.norm_biases[1]) / self.norm_values[1] * node_mask
        h_int = (h['integer'].float() -
                 self.norm_biases[2]) / self.norm_values[2]
    ...
    def unnormalize_z(self, z, node_mask):
        # Parse from z
        x, h_cat = z[:, :, 0:self.n_dims], z[:, :, self.n_dims:self.n_dims +
                                             self.num_classes]
        h_int = z[:, :, self.n_dims + self.num_classes:self.n_dims +
                  self.num_classes + 1]
    ...
    def kl_prior(self, xh, node_mask):
        ...
        kl_distance_x = gaussian_KL_for_dimension(mu_T_x,
                                                  sigma_T_x,
                                                  zeros,
                                                  ones,
                                                  d=subspace_d)
    ...
    def compute_error(self, net_out, gamma_t, eps):
        ...
        # print(eps_t.shape)
        # print(eps.shape)
        # error0 = RIOT.RIOT_sliced_batch(eps, eps_t, )
        # print(error0.shape)
        # print(error.shape)
        # print(error0.std())
        # print(error.std())
    ...
    def sample_p_xh_given_z0(self,
        ...
        h_int = z0[:, :, -1:] if self.include_charges else torch.zeros(0).to(
            z0.device)
        x, h_cat, h_int = self.unnormalize(x, z0[:, :, self.n_dims:-1], h_int,
                                           node_mask)

        h_cat = F.one_hot(torch.argmax(h_cat, dim=2),
                          self.num_classes) * node_mask
    ...
    def log_pxh_given_z0_without_constants(self,
                                           x,
                                           h,
                                           z_t,
                                           gamma_0,
        ...
        z_h_cat = z_t[:, :, self.
                      n_dims:-1] if self.include_charges else z_t[:, :,
                                                                  self.n_dims:]
        z_h_int = z_t[:, :,
                      -1:] if self.include_charges else torch.zeros(0).to(
                          z_t.device)
        ...
        h_integer = torch.round(h['integer'] * self.norm_values[2] +
                                self.norm_biases[2]).long()
        onehot = h['categorical'] * self.norm_values[1] + self.norm_biases[1]

        estimated_h_integer = z_h_int * self.norm_values[2] + self.norm_biases[
            2]
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        eps = self.sample_combined_position_feature_noise(n_samples=x.size(0),
                                                          n_nodes=x.size(1),
                                                          node_mask=node_mask)
        ...
        diffusion_utils.assert_mean_zero_with_mask(z_t[:, :, :self.n_dims],
                                                   node_mask)
        ...
        # Compute the error.
        # print('error')
        error_ = self.compute_error(net_out, gamma_t, eps)
        error0 = RIOT_batch.RIOT_sliced_batch(net_out[:, :, :self.n_dims], eps[:, :, :self.n_dims]) / 300000
        # error0 = RIOT_batch.RIOT_sliced_batch(xh[:, :, :self.n_dims],
        #                                 (z_t[:, :, :self.n_dims] -
        #                                  sigma_t * net_out[:, :, :self.n_dims]) /
        #                                 alpha_t) /2000
        # print('error1')
        #error1 = self.compute_error(net_out[:, :, self.n_dims:], gamma_t, eps[:, :, self.n_dims:])#*1.5
        # error1 = RIOT.sliced_wasserstein_distance(net_out[:, :, self.n_dims:], eps[:, :, self.n_dims:])
        #error = RIOT.SFGW_batch(xh, (z_t - sigma_t * net_out) / alpha_t, rep = 30)
        # error = RIOT.SFGW_batch(eps, net_out, rep=30)
        error = 0.9*error_ + 0.1*error0 #+ 0.1 * error1
        # print('error_', error.mean())
        # print('error0', error0.mean())
        # print('error1', error1.mean())
        # print('error01', error01.mean())
        ...
        if self.training and self.loss_type == 'l2':
            ...
            SNR_weight = (self.SNR(gamma_s - gamma_t) -
        ...
        if t0_always:
            ...
            loss_t = loss_term_0 * t_is_zero.squeeze() + t_is_not_zero.squeeze(
            ) * loss_t_larger_than_zero
        ...
        assert len(
            loss.shape) == 1, f'{loss.shape} has more than only batch dim.'
    ...
    def forward(self, x, h, node_mask=None, edge_mask=None, context=None):
        ...
        if self.training:
            ...
            loss, loss_dict = self.compute_loss(x,
                                                h,
                                                node_mask,
                                                edge_mask,
                                                context,
    ...
    def sample_p_zs_given_zt(self,
        ...
        # Compute mu for p(zs | zt).
        diffusion_utils.assert_mean_zero_with_mask(zt[:, :, :self.n_dims],
                                                   node_mask)
        diffusion_utils.assert_mean_zero_with_mask(eps_t[:, :, :self.n_dims],
                                                   node_mask)
        mu = zt / alpha_t_given_s - (sigma2_t_given_s / alpha_t_given_s /
                                     sigma_t) * eps_t
        ...
        zs = torch.cat([
            diffusion_utils.remove_mean_with_mask(
                zs[:, :, :self.n_dims], node_mask), zs[:, :, self.n_dims:]
        ],
                       dim=2)
        return zs

    def sample_combined_position_feature_noise(self, n_samples, n_nodes,
                                               node_mask):
        """
        ...
        z_x = utils.sample_center_gravity_zero_gaussian_with_mask(
            ...
            node_mask=node_mask)
        z_h = utils.sample_gaussian_with_mask(size=(n_samples, n_nodes,
                                                    self.in_node_nf),
                                              device=node_mask.device,
                                              node_mask=node_mask)
    ...
    def sample(self,
        ...
        diffusion_utils.assert_mean_zero_with_mask(z[:, :, :self.n_dims],
                                                   node_mask)
        ...
        for s in reversed(range(0, self.T)):
            ...
            z = self.sample_p_zs_given_zt(s_array,
                                          t_array,
                                          z,
                                          node_mask,
                                          edge_mask,
        ...
        x, h = self.sample_p_xh_given_z0(z,
                                         node_mask,
                                         edge_mask,
                                         context,
                                         fix_noise=fix_noise)
    ...
    def sample_chain(self,
        ...
        for s in reversed(range(0, self.T)):
            ...
            z = self.sample_p_zs_given_zt(s_array, t_array, z, node_mask,
                                          edge_mask, context)
        ...
        diffusion_utils.assert_mean_zero_with_mask(x[:, :, :self.n_dims],
                                                   node_mask)
